...
namespace PagoElectronico.Home
{
    ...
    class Home2
    {
        ...
        {
            String query = "select distinct nombre from qwerty.bancos";
            DataTable dt = db.select_query(query);
            return dt;
        }
        ...
        public int asociarTarjetaACliente(string username, string numeroTarjeta, string fechaEmision , string fechaVencimiento ,  string codigo, string emisorNombre, string numeroCuenta )
        {
            ...
            String queryBanco = "select banco_id from qwerty.cuentas where numero_cuenta = "+ numeroCuenta +" and cliente_id = "+ numeroTitular +";";
            ...
            string query = "INSERT INTO qwerty.tarjetas_de_credito VALUES(" + numeroTarjeta + "," + bancoId + ", " + emisorId + ", convert(datetime,'" + fechaEmision + "',121),convert(datetime,'" + fechaVencimiento + "',121),'" + codigo + "', " + numeroCuenta + "," + numeroTitular + ",'S');";
        ...
        public int updateTarjeta(string username, string numeroTarjeta, string emisorNombre, string fechaEmision, string fechaVencimiento, string codigo)
        {
            
            string emisor = "";

            String queryEmisor = "select emisor_id from qwerty.emisores_de_tarjetas where nombre = '" + emisorNombre + "';";
            DataTable dtemisor = db.select_query(queryEmisor);

            foreach (DataRow row in dtemisor.Rows)
            {
                emisor = row["emisor_id"].ToString();
            }

            string query = "UPDATE qwerty.tarjetas_de_credito SET numero_tarjeta = " + numeroTarjeta + ",emisor_id = " + emisor + ",fecha_emision = convert(date,'" + fechaEmision + "',121) ,fecha_vencimiento = convert(datetime,'" + fechaVencimiento + "',121),cod_seguridad = '" + codigo + "' WHERE numero_tarjeta = " + numeroTarjeta + ";";
        ...
        public DataTable getCuentas(string username)
        {
            ...
            String query = "select distinct numero_cuenta from qwerty.cuentas where cliente_id = '"+idCliente+"';";
            DataTable dt = db.select_query(query);
            return dt;
        }
        ...
        public DataTable getBancos(string username)
        {
            DataRow cliente = this.getClient(username);
            string idCliente = cliente["cliente_id"].ToString();
            String query = "select distinct banco_id from qwerty.cuentas where cliente_id = '" + idCliente + "';";
            DataTable dt = db.select_query(query);
            return dt;
        }
        ...
        {
            String query = "select t.numero_tarjeta , e.nombre , t.fecha_emision , t.fecha_vencimiento , t.cod_seguridad from qwerty.tarjetas_de_credito t join qwerty.emisores_de_tarjetas e on e.emisor_id = t.emisor_id where t.numero_tarjeta = " + numeroTarjeta + ";";
        ...
        {
            String query = "UPDATE qwerty.tarjetas_de_credito SET vinculada = 'N' WHERE numero_tarjeta = " + numeroTarjeta + ";";
            db.update_query(query);
            return 1;
        }
        ...
        public string insertarCliente(string nombre, string apellido, string mail, string documento , string tipoDoc, string pais, string calle, string altura , string piso, string depto, string localidad, string nacionalidad, string fechaNacimieno, string username, string password, string preguntaSecreta, string respuestaSecreta)
        {
            ...
            string fechaCreacion = fechaCreacionD.ToString();
            string fechaModificacion = fechaModificacionD.ToString();
