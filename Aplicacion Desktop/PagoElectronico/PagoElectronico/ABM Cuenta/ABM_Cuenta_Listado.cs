...
namespace PagoElectronico.ABM_Cuenta
{
    public partial class ABM_Cuenta_Listado : Form
    {
        public ABM_Cuenta_Listado(User user)
        {
            ...
            if (user.getRol() == "Administrador") { qeri = "select c.numero_cuenta as Cuenta,cli.nombre_usuario as Usuario,c.saldo as Saldo,c.fecha_apertura,c.fecha_cierre,m.descripcion as Moneda,p.desc_pais as País,cc.descripcion as Categoría, c.estado_id as Estado, c.banco_id from qwerty.cuentas c, qwerty.paises p, qwerty.monedas m, qwerty.categorias_de_cuentas cc,qwerty.clientes cli  where  m.moneda_id=c.moneda_id and cc.categoria_id=c.categoria_id and c.cod_pais=p.cod_pais  and c.cliente_id=cli.cliente_id"; }
            else { qeri = "select c.numero_cuenta as Cuenta,c.saldo as Saldo,c.estado_id as Estado,c.fecha_apertura,c.fecha_cierre,m.descripcion as Moneda,p.desc_pais as País,cc.descripcion as categoría, c.banco_id from qwerty.cuentas c, qwerty.paises p, qwerty.monedas m, qwerty.categorias_de_cuentas cc  where c.cliente_id=" + this.id_cliente + " and m.moneda_id=c.moneda_id and  p.cod_pais=c.cod_pais and cc.categoria_id=c.categoria_id"; }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            Modificacion_Cuenta rml = new Modificacion_Cuenta(nro_cta, this.dataGridView1.CurrentRow.Cells[5].Value.ToString(), this.dataGridView1.CurrentRow.Cells[7].Value.ToString(), this.dataGridView1.CurrentRow.Cells[6].Value.ToString(), Convert.ToInt32(this.dataGridView1.CurrentRow.Cells[8].Value.ToString()));
